...
void cla(char *first_num, char *second_num, char *converted_hex, int chunksize);
...
int main(int argc , char **argv) {
    ...
    cla(sub_first_num, sub_second_num, converted_hex, chunks);
...
void cla(char *binary_first_num, char *binary_second_num, char *converted_hex, int chunksize){
...
void super_section_carry(unsigned int *ssgl, unsigned int *sspl, unsigned *sscl){
    int l =0, ct;
    for(l=0; l<pow(BLKSIZE,1); l=l+BLKSIZE){
        if(l==0){
            sscl[l] = ssgl[l] | (sspl[l] & 0);
        }
        else{
            sscl[l] = ssgl[l] | (sspl[l] & sscl[(l+1)/BLKSIZE-1]);
        }
